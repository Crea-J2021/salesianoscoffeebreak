...
if(isset($_POST['Registrar'])){
    ...
    if(isset($imagen) && $imagen != "" && isset($nombre) && $nombre != "" && isset($precio) && $precio != "" && isset($desc) && $desc != "" && isset($cafetin) && $cafetin != ""){
       ...
       if( !((strpos($tipo,'gif') || strpos($tipo,'jpeg') || strpos($tipo,'webp')))){
         ...
         $query = "INSERT INTO menus (imagen, nombre, precio, descripcion, cafetin) values('$imagen', '$nombre', '$precio', '$desc', '$cafetin')";
         $resultado = mysqli_query($conn,$query);
         if($resultado){
              move_uploaded_file($temp,'../../../media/imagenes-menus/'.$imagen);   
